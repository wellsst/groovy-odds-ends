...
String webFiles = "rb|json|js|ts|ts|css|html|jsp|less|css|scss|tag|tld|rdf"
...
def duration = GroovyUtils.withTiming { ->
    new File(startFrom).traverse(
            ...
            type: FILES,
            maxDepth: -1,
            preDir: { if (['.svn', 'Maven', 'target']*.matches(it.name).contains(true)) return FileVisitResult.SKIP_SUBTREE },) { file ->
        ...
        if (file.size() <= maxFileSize) {
            ...
            contents.eachWithIndex { line, lineIndex ->
                ...
                def searchCondition = searchWordBoundaries ? line ==~ /(?i).*\b${searchFor}\b.*/ : line.toLowerCase().contains(searchFor.toLowerCase())
                if (searchCondition) {
                    found = true
                    matches++
            ...
            if (found) {
                ...
                println file
                println linesFound
                println "   -- files searched: ${filesSearched} : ${GroovyUtils.humanReadableByteCount(kbSearched)} in ${timeDuration}, linesSearched: ${df.format(linesSearched)} "
                println()
...
println "   -- Total files searched: ${filesSearched} : ${GroovyUtils.humanReadableByteCount(kbSearched)} , '${searchFor}' matched ${matches} times in ${timeDuration}, linesSearched: ${df.format(linesSearched)} "
String extText = ""
